...
const groupColors = (colors) => {
  return reduce(colors, (memo, colorValue, colorName) => {
    ...
    // const parsedValue = `rgba(${colorParser(colorValue).rgba.join(',')})`;
    if (memo[parsedValue]) {
...
const printColors = (colors) => {
  ...
  Object.keys(colorDictionary).sort((a,b) => {
    ...
    colorDictionary[colorValue].forEach((colorName) => {
      colorList += `
      <li>
        <span style="display: table-cell; vertical-align: bottom; color: white; background-color:rgba(${colorValue}); width: 150px; height: 150px; border: 1px solid gray;">
        </span>
        ${colorName}
      </li>
      `;
